export default {
  ...
  */
   ...
   */
  isSshRepositoryURL (url) {
    const gitUrlReg = /^git@[^:]+:[^/]+?\/.*?.git$/

    const isUrl = gitUrlReg.test(url)
    if (isUrl) {
      return true
    } else {
      return false
   ...
   * @returns true: 是, false: 否
   */
  isHttpRepositoryUrl (url) {
    const httpRepositoryUrlReg = /^http(s)?:\/\/([^/]+?\/){2}.*?.git$/
    const isUrl = httpRepositoryUrlReg.test(url)
    if (isUrl) {
      return true
    } else {
      return false
